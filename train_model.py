...
print(f"✅ Savings Model trained! Error: ₹{mean_absolute_error(y_test, savings_model.predict(X_test)):.2f}")
...
X_train3, X_test3, y_train3, y_test3 = train_test_split(X, y_corpus, test_size=0.2, random_state=42)
corpus_model = RandomForestRegressor(n_estimators=100, random_state=42)
corpus_model.fit(X_train3, y_train3)
print(f"✅ Corpus Model trained! Error: ₹{mean_absolute_error(y_test3, corpus_model.predict(X_test3)):.2f}")
